...
//		fcolor(Color::invisible) { }
    ...
    public:
        ...
        virtual void move(Point pp);	// move the shape +=dx and +=dy
...
//	void add(Point p) { Shape::add(p); }
    ...
    struct Polygon : Closed_polyline {	// closed sequence of non-intersecting lines
        ...
        void move (Point p){
            if (number_of_points() == 3) { // implemented only for equilateral triangle
                double angle = PI / 180 * 30;
                int delta = abs(point(2).x - point(1).x) / 2  / cos(angle) + 0.5;
                cout << delta << " " << cos(angle);
                p.x += delta; p.y += delta;
                points.clear();

                add(Point{p.x, p.y});
                add(Point{p.x - int(100 * cos(angle)), p.y + int(100 * sin(angle))});
                add(Point{p.x + int(100 * cos(angle)), p.y + int(100 * sin(angle))});
            }
        }
        void add(Point p);
...
*/
    ...
    struct Image : Shape {
        ...
        void move(Point pp) { Shape::move(pp); p->draw(point(0).x,point(0).y); }
    private:
...
struct Regular_hexagon : Graph_lib::Polygon {
    Regular_hexagon(Graph_lib::Point centerp, int ra) : center_point{centerp }, rad{ra }
    {
        if (rad <= 0) error("Bad rectangle: non-positive side");
        for (int i = 0; i < 6; ++i) {
            double angle = PI / 180 * (60 * i - 30);
            add(Graph_lib::Point{int(center_point.x + rad * cos(angle)), int(center_point.y + rad * sin(angle)) });
        }
    }
    int radius() const { return rad; }
    Graph_lib::Point center() const { return center_point; }
    void move (Graph_lib::Point p){
        points.clear();
        center_point = Graph_lib::Point{p.x + rad, p.y + rad};
        for (int i = 0; i < 6; ++i) {
            double angle = PI / 180 * (60 * i - 30);
            add(Graph_lib::Point{ int(center_point.x + rad * cos(angle)), int(center_point.y + rad * sin(angle)) });
        }
